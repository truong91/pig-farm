...
global {
	...
	init {
		...
		speed <- 45;
		create TransmitDiseasePig from: pigs with: [feeding_regime::2];
	...
	reflex log when: mod(cycle, 24 * 60) = 0 {
		ask simulations {
			...
			loop pig over: TransmitDiseasePig {
				...
				to: "../includes/output/transmit-fixeddiet/" + experiment_id + "-" + string(pig.id) + ".csv" rewrite: false format: "csv";
			}

			save [floor(cycle / (24 * 60)), total_CO2_emission, total_CH4_emission] to: "../includes/output/transmit-fixeddiet/" + experiment_id + "-emission-fixeddiet" + ".csv" rewrite: false format: "csv";
		}
