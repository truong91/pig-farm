	...
	reflex log when: mod(cycle, 24 * 60) = 0 {
		ask simulations {
			...
			loop pig over: FoodDiseasePigDC {
				...
				to: "../includes/output/dc-fixeddiet/" + experiment_id + "-" + string(pig.id) + ".csv" rewrite: false format: "csv";
			}

			save [floor(cycle / (24 * 60)), total_CO2_emission, total_CH4_emission] to: "../includes/output/dc-fixeddiet/" + experiment_id + "-emission-fixeddiet" + ".csv" rewrite: false format: "csv";
		}
