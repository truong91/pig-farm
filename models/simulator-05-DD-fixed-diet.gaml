...
global {
	...
	init {
		...
		speed <- 45;
		create FoodDiseasePigDD from: pigs with: [feeding_regime::2];
...
experiment DDFixedDiet {
	...
	reflex log when: mod(cycle, 24 * 60) = 0 {
		ask simulations {
			...
			save [floor(cycle / (24 * 60)), total_CO2_emission, total_CH4_emission] to: "../includes/output/dd-fixeddiet/" + experiment_id + "-emission-fixeddiet" + ".csv" rewrite: false format: "csv";
		}
